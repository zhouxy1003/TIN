...
class Model(object):
    ...
    def __init__(self, item_count, cate_count, cate_list):
        ...
        h_pos_self = tf.nn.embedding_lookup(position_emb_w, self.pos_i)
        h_emb = tf.add(h_emb, h_pos_self)

        # -- self ends ---

        hist = bert4rec_attention(h_emb, self.sl, 1, hidden_units)
        ...
        # -- fcn begin -------
        sas_i = tf.concat([u_emb_i, i_emb, u_emb_i * i_emb], axis=-1)
        sas_i = tf.layers.batch_normalization(inputs=sas_i, name='b1')
        d_layer_1_i = tf.layers.dense(sas_i, 80, activation=tf.nn.sigmoid, name='f1')
        d_layer_2_i = tf.layers.dense(d_layer_1_i, 40, activation=tf.nn.sigmoid, name='f2')
        d_layer_3_i = tf.layers.dense(d_layer_2_i, 1, activation=None, name='f3')

        sas_j = tf.concat([u_emb_j, j_emb, u_emb_j * j_emb], axis=-1)
        sas_j = tf.layers.batch_normalization(inputs=sas_j, name='b1', reuse=True)
        d_layer_1_j = tf.layers.dense(sas_j, 80, activation=tf.nn.sigmoid, name='f1', reuse=True)
...
def bert4rec_attention(E, keys_length, h, emb):
    ...
    paddings = tf.zeros_like(outputs)
    outputs = tf.where(key_masks, outputs, paddings)
    # outputs = tf.linalg.band_part(outputs, -1, 0)  # lower triangular matrix
