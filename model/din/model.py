...
class Model(object):
  ...
  def __init__(self, user_count, item_count, cate_count, cate_list, predict_batch_size, predict_ads_num):
    ...
    '''
    hist_i, attention1, queries, keys = attention(i_emb, h_emb, self.sl)
    self.queries = queries
    self.keys = keys
    #-- attention end ---
    # hist_i = sum_pooling(h_emb, self.sl)
    ...
    hist_j, attention2, queries2, keys2 = attention(j_emb, h_emb, self.sl)
    ...
    item_emb_sub = item_emb_all[:predict_ads_num,:]#取0-predict_ads_num所有列的数据
    self.item_emb_sub = item_emb_sub
    ...
    # hist_sub = tf.Print(hist_sub, [hist_sub], message="This is a: ")
    #-- attention end ---
    ...
    # din_sub = tf.concat([item_emb_sub], axis=-1)
    din_sub = tf.layers.batch_normalization(inputs=din_sub, name='b1', reuse=True)
    d_layer_1_sub = tf.layers.dense(din_sub, 80, activation=tf.nn.relu, name='f1', reuse=True) # bias use later
  ...
  def train(self, sess, uij, l):
    loss, _, attention1, keys, queries  = sess.run([self.loss, self.train_op, self.attention1, self.keys, self.queries], feed_dict={
        self.u: uij[0],
        self.i: uij[1],
        self.y: uij[2],
        self.hist_i: uij[3],
    ...
    return loss, attention1, keys, queries
...
def attention(queries, keys, keys_length):
  ...
  # queries1 = tf.zeros(queries.shape)
  # din_all = tf.concat([queries, keys, queries-keys, queries*keys], axis=-1)
  # din_all = tf.concat([queries], axis=-1) 
  din_all = tf.concat([keys], axis=-1)
  # din_all = tf.concat([queries*keys], axis=-1)
  d_layer_1_all = tf.layers.dense(din_all, 80, activation=tf.nn.relu, name='f1_att', reuse=tf.AUTO_REUSE)
  d_layer_2_all = tf.layers.dense(d_layer_1_all, 40, activation=tf.nn.relu, name='f2_att', reuse=tf.AUTO_REUSE)
  ...
  '''
  with tf.variable_scope('f1_att', reuse=True):
       w1 = tf.get_variable('kernel') 
       b1 = tf.get_variable('bias')
  with tf.variable_scope('f2_att', reuse=True):
       w2 = tf.get_variable('kernel')
  with tf.variable_scope('f3_att', reuse=True):
       w3 = tf.get_variable('kernel')
  '''
  
  
  # outputs = tf.reduce_sum(tf.multiply(queries, keys),axis=-1)
  # outputs = tf.reshape(outputs, [-1, 1, tf.shape(keys)[1]])  
  # outputs = tf.zeros(tf.shape(outputs))
  ...
  return outputs, attention1, queries, keys
...
def attention_multi_items(queries, keys, keys_length):
  ...
  # outputs = tf.Print(outputs, [outputs], message="This is a: ")
