...
random.seed(2021)  # 2021
np.random.seed(2021)  # 2021
tf.set_random_seed(2021)  # 2021
...
with open(
        '/cephfs/group/file-teg-datamining-wx-dm-intern/lukahlzhou/ft_local/DeepInterestNetwork-master/din/ft_local/dataset1.pkl',
        'rb') as f:
    ...
    for i in range(len(train_set)):
        ...
        l = len(tmp[1])
        a = (tmp[0], tmp[1], tmp[2], tmp[3], random.sample(list, l))
        all_train.append(a)
    train_set = all_train
    ...
    for _ in range(20):
        ...
        for _, uij in DataInput(train_set, train_batch_size):
            ...
            if model.global_step.eval() % 1000 == 0:
                ...
                print('Epoch %d Global_step %d\tTrain_loss: %.4f\tEval_GAUC: %.4f\tEval_AUC: %.4f\tBest_Logloss: %.4f\tBest_GAUC: %.4f' %
                      (model.global_epoch_step.eval(), model.global_step.eval(),
                       loss_sum / 1000, eval_gauc, Auc, final_test_logloss, final_test_gauc))
                sys.stdout.flush()
                loss_sum = 0.0
